...
BEGIN
	...
	FROM
		PROCEDO_CLIENTE A 
        INNER JOIN PROCEDO_CIDADE B ON A.CIDADE = B.CODIGO
        INNER JOIN PROCEDO_CLIENTE_ORIGEM C ON A.CODIGO = C.CODIGO_CLIENTE AND (pORIGEM IS NULL OR (FIND_IN_SET(C.CODIGO_ORIGEM, pORIGEM)))
	WHERE
...
-- Dump completed on 2021-12-08 11:29:53
